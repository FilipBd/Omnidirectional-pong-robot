...
class Robot:
	...
	def wheels(self,X,Y,R, radius = 1):
		...
		if R == None:
			R = 0
		self.w1 = (-1/2*(X*1.0)) - (math.sqrt(3)/2*Y) + (radius*R)
		self.w2 = (-1/2*(X*1.0)) + (math.sqrt(3)/2*Y) + (radius*R)
...
m1_speed, m2_speed, m3_speed = robot.wheels(100,0,0)
# mellan -100 och 100
...
while True:
	...
	e1_error = b1*math.cos(degreeError)
	e2_error = b2*math.cos(degreeError)
	e3_error = b3*math.cos(degreeError)
